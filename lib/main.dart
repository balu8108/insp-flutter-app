  ...
  void initState() {
    ...
    final riskMonitoringCallback = RiskMonitoringCallback(
      onDeveloperModeStatusChanged: (bool status) {
        if(status){
          _showDialog(const ADBEnablePopup(message: 'Please turn off developer mode to continue'),);
        }
      ...
      onDeviceRootedOrJailBroken: () {
        _showDialog(const ADBEnablePopup(message: 'Device is rooted or jailbroken!'),);
        //print('Device has been rooted!');
      },
      onBootLoaderUnlocked: () {
        _showDialog(const ADBEnablePopup(message: 'Bootloader is unlocked!'),);
        //print('Bootloader has been unlocked!');
      },
      onMockGpsStatusChanged: (bool status) {
        if(status){
          _showDialog(const ADBEnablePopup(message: 'Please turn off mock GPS to continue'),);
        }
      ...
      onVPNStatusChanged: (bool status) {
        if(status){
          _showDialog(const ADBEnablePopup(message: 'Please turn off vpn to continue'),);
        }
        //print('VPN status changed: $status');
      },
      onDebuggerStatusChanged: (bool status) {
        if(status){
          _showDialog(const ADBEnablePopup(message: 'Please turn off debugger to continue'),);
        }
        //print('Debugger status changed: $status');
      },
      onHookingStatusChanged: (bool status) {
        if(status){
          _showDialog(const ADBEnablePopup(message: 'Please turn off hooking to continue'),);
        }
